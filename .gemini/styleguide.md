...
| FE SDK generation | `@hey-api/openapi-ts` (consumes `/openapi.json`)        |
...
│   ├── auth.middleware.ts     # requireSession(), requirePermission()
...
export const requirePermission =
  (resource: string, actions: string[]): RequestHandler =>
  async (req, res, next) => {
